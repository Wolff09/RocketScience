...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < loops; i++)
		sts.addTransition(0, 0,
			xp[i-1] * xq[i],
			!np[i-1] * !nq[i] * np[i] * nq[i-1] * id_all_but(i-1, i, one)
		);

	// last petri transitions
	sts.addTransition(0, 0,
		xp[loops-1],
		!np[loops-1] * nq[loops-1] * id_all_but(loops-1, loops-1, one)
	);


	BDD init = sts.stateCurrent(0);
	for (int i = 0; i < loops; i++) init *= xp[i] * !xq[i];
